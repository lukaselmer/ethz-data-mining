...
class FiFoQueue():
    ...
    def check_avoidasdfasdfasdfa(self, k, article_stats):
        a = article_stats[k][0]
        b = article_stats[k][1]
        ratio = 1.0
        if a + b > 0:
...
class HideSomeLinUCB:
    ...
    def recommend(self, timestamp, user_features, articles):
        ...
        best_article = self.current_article
        best_ucb = -np.inf
        for article in self.filter_bad(articles):
        ...
        self.current_article = best_article

        self.current_user = user_features
        return self.current_article

    def filter_bad(self, articles):
        if self.bad_log.has_key(self.count):
            filter_article = self.bad_log[self.count]
            articles = filter(articles, lambda a: filter_article is not a)
        return articles

    def do_update(self, reward, article):
        #self.ucb_stats['lines_evaluated'] += 1
        #self.article_stats[article][reward] += 1
    ...
    def update(self, reward):
        ...
        if reward == 0 or reward == 1:
            if reward == 0:
                # TODO: self.bad_log[self.count] = self.current_article
                print "%i %i" % (self.count, self.current_article)
            self.do_update(reward, self.current_article)
